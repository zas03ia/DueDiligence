...
class GroqLLMService:
    ...
    def generate_single_answer_with_citations(self, question: str, 
    ...
    def batch_generate_answers(self, questions_contexts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _prepare_context(self, context: List[str]) -> str:
        ...
        if len(context_text) > max_context_length:
            ...
            for chunk in context:
                if current_length + len(chunk) + 10 <= max_context_length:  # +10 for separator
    ...
    def _get_system_prompt(self, question_type: str) -> str:
        ...
        if question_type == "BOOLEAN":
            return base_system + " For yes/no questions, always start with a clear 'Yes' or 'No' answer."
        
        elif question_type == "NUMERIC":
            return base_system + " For numerical questions, provide specific numbers with units when applicable."
        
        elif question_type == "DATE":
            return base_system + " For date questions, provide specific dates in a clear format."
    ...
    def _parse_evaluation_fallback(self, evaluation_text: str) -> Dict[str, Any]:
        ...
        confidence_match = re.search(r'confidence[_\s]*score[:\s]*([0-9.]+)', evaluation_text.lower())
        if confidence_match:
            try:
        ...
        answerable_match = re.search(r'answerable[:\s]*(yes|no|true|false)', evaluation_text.lower())
        if answerable_match:
