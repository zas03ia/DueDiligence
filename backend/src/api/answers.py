...
async def confirm_answer(
    ...
    try:
        ...
        success = answer_service.confirm_answer(str(answer.project_id), str(answer.question_id))
...
async def get_project_answers_with_context(
    ...
    try:
        ...
        for answer in answers:
            context_data = answer_service.get_answer_with_context(project_id, str(answer.id))
...
async def compare_ai_vs_manual_answers(
    project_id: str,
    db: Session = Depends(get_db)
):
...
@router.get("/question/{question_id}/project/{project_id}", response_model=Dict[str, Any])
