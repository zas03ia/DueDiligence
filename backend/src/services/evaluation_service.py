...
class EvaluationService:
    ...
    def evaluate_project_answers(
        self, project_id: str, ground_truth_answers: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            answers = (
                self.db.query(Answer).filter(Answer.project_id == project_id).all()
            )
            if not answers:
            ...
            for answer in answers:
                ...
                if ground_truth:
                    ...
                    total_similarity += evaluation.get("similarity_score", 0.0)
                    total_confidence += answer.confidence_score or 0.0
            ...
            overall_score = (
                total_similarity / num_evaluations if num_evaluations > 0 else 0
            )
    ...
    def evaluate_single_answer(
        self, answer: Answer, ground_truth: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            semantic_similarity = self._calculate_semantic_similarity(
                answer_to_evaluate, ground_truth
            )
    ...
    def _combine_similarity_scores(
        self, semantic: float, keyword: float, length: float
    ) -> float:
    ...
    def _assess_answer_quality(
        self, answer: str, ground_truth: str, similarity_score: float
    ) -> Dict[str, Any]:
        ...
        if similarity_score >= 0.8:
            ...
            quality = "fair"
            description = (
                "Answer has some similarity to ground truth but significant differences"
            )
        else:
            quality = "poor"
    ...
    def _generate_evaluation_report(
        ...
        quality_counts = Counter(
            [eval["quality_assessment"]["quality"] for eval in evaluations]
        )

        # Calculate statistics
        similarity_scores = [
            eval["similarity_scores"]["combined"] for eval in evaluations
        ]
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                overall_score, quality_counts
            ),
        }

    def _generate_recommendations(
        self, overall_score: float, quality_counts: Counter
    ) -> List[str]:
        ...
        if quality_counts.get("poor", 0) > len(quality_counts) * 0.3:
            recommendations.append(
                "Significant number of poor quality answers - review indexing strategy"
            )

        if quality_counts.get("excellent", 0) > len(quality_counts) * 0.7:
            recommendations.append(
                "High quality answers achieved - current approach is working well"
            )

        if overall_score > 0.7 and overall_score < 0.8:
            recommendations.append(
                "Good performance - consider fine-tuning confidence thresholds"
            )
    ...
    def _calculate_similarity_metrics(self, evaluations: List[Dict]) -> Dict[str, Any]:
        ...
        semantic_scores = [
            eval["similarity_scores"]["semantic"] for eval in evaluations
        ]
    ...
    def compare_ai_vs_manual_answers(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            answers = (
                ...
                .filter(
                    Answer.project_id == project_id, Answer.manual_answer.isnot(None)
                )
                .all()
            )
